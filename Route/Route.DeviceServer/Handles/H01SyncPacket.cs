...
{
    /// <summary>
    ///     lớp xử lý thông tin packet 01
    /// </summary>
    [Export(typeof (IDeviceHandlePacket))]
    [PartCreationPolicy(CreationPolicy.Shared)]
    [DeviceOpCode(1)]
    public class H01SyncPacket : BaseHandle, IDeviceHandlePacket
    {
        [Import] private ILog _log;

        public Delegate GetHandle()
        {
            return new Action<IClient, P01SyncPacket>(Handle);
        }

        private void Handle(IClient client, P01SyncPacket p)
        {
            ...
            _log.Debug("PACKET",
                $"Chuyển tiếp thông tin qua máy chủ xử lý : {(ForwardSysn(p.Serial, p) ? "thành công" : "thất bại")}");
        }

        public void Handle301(IClient client, P01SyncPacket p,bool last)
        {
            _log.Warning("PACKET", $"Serial : {p.Serial}");
            _log.Warning("PACKET", $"Time : {p.Time.ToString("G")}");
            _log.Warning("PACKET", $"GpsStatus : {p.GpsStatus}");
            ...
            _log.Warning("PACKET", $"OverTimeInDay: {p.TimeWorkInDay}");
            _log.Debug("PACKET",
                $"Chuyển tiếp thông tin qua máy chủ xử lý : {(ForwardSysn301(p.Serial, p,last) ? "thành công" : "thất bại")}");
        }
