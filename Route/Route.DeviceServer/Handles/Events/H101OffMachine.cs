...
namespace Route.DeviceServer.Handles.Events
{
    ...
    [DeviceOpCode(101)]
    public class H101OffMachine : BaseHandle, IDeviceHandlePacket
    {
        [Import] private ILog _log;

        public Delegate GetHandle()
        {
            return new Action<IClient, P101OffMachine>(Handle);
        }

        private void Handle(IClient client, P101OffMachine p)
        {
            ...
            _log.Debug("PACKET",
                $"Chuyển tiếp thông tin tắt máy qua máy chủ xử lý : {(ForwardOffMachine(p.Serial, p) ? "thành công" : "thất bại")}");
        }
    }
}