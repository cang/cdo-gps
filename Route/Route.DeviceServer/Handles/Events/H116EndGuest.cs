...
namespace Route.DeviceServer.Handles.Events
{
    ...
    [DeviceOpCode(116)]
    public class H116EndGuest : BaseHandle, IDeviceHandlePacket
    {
        [Import] private ILog _log;

        public Delegate GetHandle()
        {
            return new Action<IClient, P116EndGuest>(Handle);
        }

        private void Handle(IClient client, P116EndGuest p)
        {
            ...
            _log.Debug("PACKET",
                $"Chuyển tiếp thông tin mở máy qua máy chủ xử lý : {(ForwardEndGuest(p.Serial, p) ? "thành công" : "thất bại")}");
        }
